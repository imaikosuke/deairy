...
import (
	...
	"github.com/imaikosuke/deairy/backend/env"
	"github.com/imaikosuke/deairy/backend/graph"
	mid "github.com/imaikosuke/deairy/backend/middleware"
	"github.com/imaikosuke/deairy/backend/pkg/errors"
	gormpkg "github.com/imaikosuke/deairy/backend/pkg/gorm"
...
func realMain() error {
	...
	config := graph.Config{Resolvers: graph.NewResolver(diarySvc, userSvc)}
	...
	// GraphQL Endpoint
	http.Handle("/v1/graphql", setupGlobalMiddleware(srv, userSvc, diarySvc))

	addr := fmt.Sprintf("%s:%s", env.Get().Server.APIHost, env.Get().Server.APIPort)
	app.Logger.Info().Msgf("Serving deairy on http://%s", addr)
...
func setupGlobalMiddleware(handler http.Handler, userSvc service.User, diarySvc service.Diary) http.Handler {
	a := alice.New()
	a = a.Append(mid.Recover)
	a = a.Append(mid.WithContext)
	a = a.Append(mid.CORS().Handler)
	a = a.Append(mid.LoaderMiddleware(userSvc, diarySvc))
